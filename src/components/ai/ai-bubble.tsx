...
function getSingleParam(sp: URLSearchParams, key: string) {
  const v = sp.get(key);
  return v && v.trim() ? v.trim() : undefined;
}

function getAllFilters(sp: URLSearchParams): Record<string, string | string[]> {
  ...
  for (const key of sp.keys()) {
    ...
    const values = sp.getAll(key).map((v) => v.trim()).filter(Boolean);
...
function inferPageContext(pathname: string, sp: URLSearchParams): PageContext {
  const filters = getAllFilters(sp);

  // Conservative param inference.
  // This is safe now and gets stronger later as pages adopt URL contracts.
  const contactId =
    getSingleParam(sp, "contactId") ||
    getSingleParam(sp, "contact") ||
    getSingleParam(sp, "personId") ||
    getSingleParam(sp, "selectedContactId");

  const listingId =
    getSingleParam(sp, "listingId") ||
    getSingleParam(sp, "listing") ||
    getSingleParam(sp, "selectedListingId");

  const taskId =
    getSingleParam(sp, "taskId") ||
    getSingleParam(sp, "task") ||
    getSingleParam(sp, "selectedTaskId");

  const conversationId =
    getSingleParam(sp, "conversationId") ||
    getSingleParam(sp, "conversation") ||
    getSingleParam(sp, "selectedConversationId");

  const tab =
    getSingleParam(sp, "tab") ||
    getSingleParam(sp, "view") ||
    getSingleParam(sp, "panelTab");

  // Light pathname heuristics. Useful even before full URL contracts exist.
  ...
  return {
    contactId: isPeople || isComms ? contactId : contactId,
    listingId: isListings ? listingId : listingId,
    taskId: isTasks ? taskId : taskId,
    conversationId: isComms ? conversationId : conversationId,
    tab,
    filters,
  };
}
...
export default function AIBubble() {
  ...
  async function callAI(nextMessages: Msg[]) {
    ...
    const pageContext = inferPageContext(pathname, sp);
  ...
  async function send(raw: string) {
    ...
    try {
      ...
      const result = await callAI(snapshot);
        ...
        <div className="fixed inset-0 z-[79]">
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, rgba(244,232,200,0.18), rgba(26,39,71,0.28), rgba(0,0,0,0))",
              }}
            />
            ...
            <div
              ref={bodyRef}
              className={["relative px-4 py-3 space-y-3", "flex-1 min-h-0", "overflow-y-auto"].join(" ")}
            >
                ...
                <div className="flex-1">
                  <textarea
                    ...
                    className={[
                      ...
                      "px-3",
                      "py-[9px]",
                      "text-sm leading-[20px] text-[#F4E8C8]",
