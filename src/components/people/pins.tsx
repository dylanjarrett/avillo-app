// src/components/people/pins.tsx
"use client";
...
export default function PeoplePins({
  contactId,
  disabled,
  onRequiresSave,
}: {
  ...
  const canUse = !!contactId && !disabled;

  // Data
  const [workspacePins, setWorkspacePins] = useState<Pin[]>([]);
  const [contactPins, setContactPins] = useState<ContactPin[]>([]);

  // Load states
  const [loadingWorkspace, setLoadingWorkspace] = useState(true);
  const [loadingContact, setLoadingContact] = useState(false); // initial load + manual refresh only
  ...
  async function loadContactPins(id: string, signal?: AbortSignal) {
    const res = await fetch(`/api/pins/attach/${id}`, { cache: "no-store", signal });
    if (!res.ok) {
      const data = await res.json().catch(() => null);
      throw new Error(data?.error || "Failed to load contact pins.");
    }
    const data = await res.json();
    const pins: ContactPin[] = Array.isArray(data?.pins) ? data.pins : [];
    setContactPins(pins);
  }
  ...
  async function refreshWorkspace(opts?: { silent?: boolean }) {
    try {
      ...
      if (!opts?.silent && contactId) setError(e?.message || "We couldn’t load pins right now.");
    }
  }

  async function refreshContact(id: string, opts?: { silent?: boolean }) {
    try {
      if (!opts?.silent) {
        setError(null);
        setLoadingContact(true);
      }
      await loadContactPins(id);
    } catch (e: any) {
      if (!opts?.silent) setError(e?.message || "We couldn’t load contact pins right now.");
    } finally {
      if (!opts?.silent) setLoadingContact(false);
    }
  }
  ...
  async function manualRefresh() {
    ...
    try {
      ...
      setError(null);
      setLoadingContact(true);
      await Promise.all([loadWorkspacePins(), loadContactPins(contactId)]);
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setError(null);
        setLoadingContact(true);
        await loadContactPins(contactId, ac.signal);
      } catch (e: any) {
        if (e?.name === "AbortError") return;
        setError(e?.message || "We couldn’t load contact pins right now.");
      } finally {
        setLoadingContact(false);
      }
    })();
  ...
  const attachedKeySet = useMemo(() => {
    return new Set(contactPins.map((p) => (p.nameKey ?? p.name.toLowerCase())));
  }, [contactPins]);
  ...
  async function attachByPinId(pinId: string) {
    ...
    setContactPins((prev) => [{ id: tempId, name: optimisticName, nameKey: optimisticKey }, ...prev]);
    setQuery("");
    setOpen(false);
    setCreateConfirmKey(null);
    ...
    try {
      ...
      const res = await fetch(`/api/pins/attach/${contactId}`, {
        method: "POST",
        headers: jsonHeaders(),
      ...
      await refreshContact(contactId, { silent: true });
      inputRef.current?.focus();
    } catch (e: any) {
      setContactPins((prev) => prev.filter((p) => p.id !== tempId));
  ...
  async function createAndAttachConfirmed(nameRaw: string) {
    ...
    // Optimistic add
    const tempId = `temp:${Date.now()}`;
    setContactPins((prev) => [{ id: tempId, name, nameKey }, ...prev]);
    setQuery("");
    setOpen(false);
    setCreateConfirmKey(null);
    ...
    try {
      ...
      await Promise.all([refreshWorkspace({ silent: true }), refreshContact(contactId, { silent: true })]);
      inputRef.current?.focus();
    } catch (e: any) {
      setContactPins((prev) => prev.filter((p) => p.id !== tempId));
  ...
  async function detach(pinId: string) {
    ...
    const snapshot = contactPins;
    setContactPins((prev) => prev.filter((p) => p.id !== pinId));

    try {
      setRemoving((p) => ({ ...p, [pinId]: true }));
      setError(null);

      const res = await fetch(`/api/pins/detach/${contactId}/${pinId}`, { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
  ...
  const showEmptyState = !!contactId && !loadingContact && contactPins.length === 0;
  const showChips = !!contactId && !loadingContact && contactPins.length > 0;
    ...
    <div className="rounded-2xl border border-slate-700/60 bg-gradient-to-b from-slate-950/60 to-slate-900/50 p-4 shadow-[0_0_40px_rgba(2,6,23,0.55)]">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          disabled={!contactId || manualRefreshing || loadingContact}
        ...
        >
          {manualRefreshing || loadingContact ? "Syncing…" : "Refresh"}
        </button>
      </div>
      ...
      {!contactId && (
        <div className="mt-4 rounded-xl border border-slate-800/70 bg-slate-950/40 p-3">
          <p className="text-[11px] text-[var(--avillo-cream-muted)]">Save this contact to start pinning.</p>
          {onRequiresSave && (
            <button
              type="button"
          ...
          <div className="mt-4">
            ...
            {query.trim() && (suggestions.length > 0 || didYouMean.length > 0) && (
              <p className="mt-2 text-[10px] text-[var(--avillo-cream-muted)]">
                {suggestions.length ? "Use ↑/↓ then Enter — or click a suggestion." : "Tap a suggestion to avoid creating a typo pin."}
              </p>
            )}
          </div>
        </>
