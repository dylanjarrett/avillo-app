...
const LISTING_CONDITION_FIELDS = [{ value: "listing.status", label: "Listing status" }];

const ALL_CONDITION_FIELDS = [...CONTACT_CONDITION_FIELDS, ...LISTING_CONDITION_FIELDS];
...
// ✅ Merge tags list used in examples + "Variables:" hints
const MERGE_TAGS = [
  "{{firstName}}",
  "{{agentName}}",
  "{{agentPhone}}",
...
const SMS_EXAMPLE =
  "Ex: Hey {{firstName}} — thanks for reaching out. Best way to connect is call/text {{agentPhone}} or email {{agentEmail}}.";
...
export default function StepModal({
  ...
  function normaliseIfConfig(raw: any): IfConfig {
    const availableFields = conditionFields.length > 0 ? conditionFields : ALL_CONDITION_FIELDS;
          ...
          : [
              {
                ...
                operator: "equals",
                value:
                  FIELD_VALUE_OPTIONS[(availableFields[0]?.value as string) || "contact.stage"]?.[0] ??
                  "",
              },
            ];
    ...
    const operator: "equals" | "not_equals" = raw?.operator === "not_equals" ? "not_equals" : "equals";
  ...
  function addConditionRow() {
    setForm((prev: any) => {
      ...
      const value =
        FIELD_VALUE_OPTIONS[field]?.[0] ?? FIELD_VALUE_OPTIONS["contact.stage"]?.[0] ?? "";
  ...
  function removeCondition(index: number) {
    setForm((prev: any) => {
      ...
      const next = [...cfg.conditions];
      next.splice(index, 1);
      return { ...cfg, conditions: next.length ? next : cfg.conditions };
    });
  }
  ...
  }, [type, initialConfig, initialThen, initialElse, conditionScope, conditionFields.length]);

  // Wait Normalizers (Branch + Main)
  function normalizeWaitConfig(raw: any) {
    const unit: "hours" | "days" | "weeks" | "months" =
      raw?.unit === "days" || raw?.unit === "weeks" || raw?.unit === "months" ? raw.unit : "hours";
  ...
  function handleSave() {
    ...
    if (type === "IF") {
      ...
      onSave({ join: cfg.join, conditions: cleaned }, normalizedThen, normalizedElse);
      return;
    }
    ...
    if (type === "WAIT") {
      ...
      const unit: "hours" | "days" | "weeks" | "months" = form.unit || "hours";
  ...
  function addBranchStep(branch: "then" | "else", stepType: StepType) {
    ...
    if (branch === "then") setThenSteps((p) => [...p, newStep]);
    else setElseSteps((p) => [...p, newStep]);
  }

  function updateBranchStep(branch: "then" | "else", id: string, patch: any) {
    const setter = branch === "then" ? setThenSteps : setElseSteps;
    setter((prev) => prev.map((s) => (s.id === id ? { ...s, config: { ...s.config, ...patch } } : s)));
  }
  ...
  const ifCfg: IfConfig | null = type === "IF" ? normaliseIfConfig(form ?? {}) : null;

  const MergeTagsHint = ({ size = "main" }: { size?: "main" | "branch" }) => {
    const cls =
      size === "branch"
        ? "text-[8px] text-[var(--avillo-cream-muted)]"
        : "mt-2 text-[9px] text-[var(--avillo-cream-muted)]";
              ...
              {type === "TASK" && (
                <div className="space-y-4">
                  <div className="rounded-xl border border-slate-700/80 bg-slate-900/70 px-3 py-3">
                    ...
                    <input
                      className="mt-1 w-full bg-transparent text-[11px] text-slate-50 outline-none placeholder:text-[var(--avillo-cream-muted)]"
                      placeholder="Ex: Call {{firstName}} at {{agentPhone}} and follow up on {{propertyAddress}}"
                      value={form.text ?? ""}
                      onChange={(e) => update("text", e.target.value)}
                    />
                  </div>
                ...
                <div className="space-y-4">
                  <div className="rounded-xl border border-slate-700/80 bg-slate-900/70 px-3 py-3">
                    ...
                    <p className="mt-1 text-[9px] text-[var(--avillo-cream-muted)]">
                      Example: <strong>4 hours</strong> ≈ later the same day. <strong>2 days</strong> ≈ 48 hours later.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  <div className="rounded-xl border border-slate-700/80 bg-slate-900/70 px-3 py-3">
                    ...
                    <div className="mt-3 space-y-2">
                      {ifCfg.conditions.map((cond, index) => {
                        const valuesForField = FIELD_VALUE_OPTIONS[cond.field] ?? [];
                        ...
                        return (
                          ...
                          >
                            <span className="text-[10px] text-[var(--avillo-cream-muted)]">If</span>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                updateCondition(index, { field: newField, value: firstVal });
                              }}
                            >
                              {conditionFields.map((f) => (
                                <option key={f.value} value={f.value} className={optionClass}>
                                  {f.label}
                                </option>
                              ))}
                            </select>
                            ...
                            {s.type === "SMS" && (
                              <div className="mt-1 space-y-1">
                                ...
                                <textarea
                                  ...
                                  value={s.config?.text ?? ""}
                                  onChange={(e) =>
                                    updateBranchStep("then", s.id, { text: e.target.value })
                                  }
                                />
                                <MergeTagsHint size="branch" />
                              </div>
                            )}
                            ...
                            {s.type === "WAIT" && (
                              <div className="mt-1 flex gap-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateBranchStep("then", s.id, { amount: e.target.value })
                                  }
                                />
                                <select
                                  ...
                                  onChange={(e) =>
                                    updateBranchStep("then", s.id, { unit: e.target.value })
                                  }
                                >
                                  <option className={optionClass} value="hours">
                                    Hours
                            ...
                            {s.type === "SMS" && (
                              <div className="mt-1 space-y-1">
                                ...
                                <textarea
                                  ...
                                  value={s.config?.text ?? ""}
                                  onChange={(e) =>
                                    updateBranchStep("else", s.id, { text: e.target.value })
                                  }
                                />
                                <MergeTagsHint size="branch" />
                              </div>
                            ...
                            {s.type === "WAIT" && (
                              <div className="mt-1 flex gap-2">
                                <input
                                  ...
                                  value={s.config?.amount ?? ""}
                                  onChange={(e) =>
                                    updateBranchStep("else", s.id, { amount: e.target.value })
                                  }
                                />
                                <select
                                  className={selectBase + " flex-1 text-[10px] px-1 py-0.5"}
                                  value={s.config?.unit ?? "hours"}
                                  onChange={(e) =>
                                    updateBranchStep("else", s.id, { unit: e.target.value })
                                  }
                                >
                                  <option className={optionClass} value="hours">
                                    Hours
